...
from fg.data.dataloader import DeepFashion2Dataset
...
def train(config: Optional[dict[str, Any]] = None) -> None:
    ...
    **checkpoint resumption** (set ``config["resume_from"]`` to a path).
    ...
    # Hold out a validation split so early stopping selects for generalisation
    # rather than train-loss overfitting (val CSV if present, else random split).
    val_loader: DataLoader | None = None
    try:
        val_dataset = DeepFashion2Dataset(
            root=cfg["data_root"], split="val", transform=model.preprocess
        )
        train_dataset: Any = dataset
    except (FileNotFoundError, ValueError):
        val_frac: float = cfg.get("val_frac", 0.1)
        n_val: int = max(1, int(len(dataset) * val_frac))
        ...
        logger.info("No val split found — random %d/%d train/val.", n_train, n_val)
    ...
    for epoch in range(start_epoch, cfg["epochs"]):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                img_emb, txt_emb = model(images, batch["text"])
                val_total += criterion(img_emb, txt_emb).item()
        val_loss: float = val_total / max(1, len(val_loader))
        logger.info(
            "Epoch %d/%d — train=%.4f val=%.4f",
            epoch + 1, cfg["epochs"], avg_loss, val_loss,
        )
        ...
        # Track best model by VALIDATION loss (not train loss)
        if val_loss < best_loss:
            best_loss = val_loss
            patience_counter = 0
            model.save(best_model_path)
            logger.info("  ↳ New best model saved (val=%.4f)", best_loss)
        else:
            patience_counter += 1
            logger.info(
